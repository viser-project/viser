...
# Public type aliases (Literal/Union/etc.) that should be documented alongside
...
def _emit(names_classes: List[str], names_aliases: List[str]) -> str:
    """Emit autoclass + autodata directives, alphabetically interleaved."""
    classes = set(names_classes)
    body = ""
    for name in sorted(names_classes + names_aliases):
        directive = "autoclass" if name in classes else "autodata"
        body += f".. {directive}:: viser.{name}\n\n"
    return body


def generate_gui_handles_rst(handles: List[str], aliases: List[str]) -> str:
...
"""
    content += _emit(handles, aliases)
    return content


def generate_events_rst(events: List[str], aliases: List[str]) -> str:
...
"""
    # Events use ``viser.X()`` (with parens) so Sphinx renders the class
    # signature as a callable; type aliases just use ``viser.X``.
    classes = set(events)
    for name in sorted(events + aliases):
        if name in classes:
            content += f".. autoclass:: viser.{name}()\n\n"
        else:
            content += f".. autodata:: viser.{name}\n\n"
    return content
...
def main():
    ...
    events_content = generate_events_rst(
        handle_classes["events"], TYPE_ALIASES["events"]
    )
    events_path = advanced_dir / "events.rst"

    print(
        f"Writing {len(handle_classes['events'])} events"
        f" + {len(TYPE_ALIASES['events'])} type aliases to {events_path}"
    )
    with open(events_path, "w") as f:
        f.write(events_content)

    # Generate GUI handles file
    gui_content = generate_gui_handles_rst(handle_classes["gui"], TYPE_ALIASES["gui"])
    gui_path = handles_dir / "gui_handles.rst"

    print(
        f"Writing {len(handle_classes['gui'])} GUI handles"
        f" + {len(TYPE_ALIASES['gui'])} type aliases to {gui_path}"
    )
    ...
    print("\nType aliases:")
    for category, names in TYPE_ALIASES.items():
        for name in names:
