...
export function FrameSynchronizedMessageHandler() {
  ...
  // Drain the message queue. Called from the render loop (below) and, via
  // viewerMutable.drainMessageQueue, from enqueueMessages() for the batches
  // that shouldn't wait for a frame.
  ...
  // handleMessage is recreated every render, so the drain closure is too;
  // the hooks below go through this ref and register once.
  const drainRef = React.useRef(drainMessageQueue);
  drainRef.current = drainMessageQueue;
  ...
  React.useEffect(() => {
    viewerMutable.drainMessageQueue = () => drainRef.current();
    return () => {
