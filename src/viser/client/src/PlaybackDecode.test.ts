...
async function decodeFixture(): Promise<SerializedMessages> {
  const bytes = await base64ToBytes(FIXTURE_B64);
...
describe("PlaybackDecode", () => {
  it("decodes a Python-serialized recording", async () => {
    ...
    const meshes = recording.messages
      .map(([, message]) => message)
  ...
  it("decodes base64 through the atob fallback identically", async () => {
    // Force the last-resort path by hiding fetch-based decoding: compare
    // against the primary path's output.
    const primary = await base64ToBytes(FIXTURE_B64);
    const binaryString = atob(FIXTURE_B64);
    const fallback = new Uint8Array(binaryString.length);
    for (let i = 0; i < binaryString.length; i++) {
      fallback[i] = binaryString.charCodeAt(i);
    }
    expect(Array.from(primary)).toEqual(Array.from(fallback));
  });
});
