...
import {
  ...
  UniformValue,
  UniformValueObj,
} from "../utils/SquareDataTexture.js";
...
declare module "../InstancedMesh2.js" {
  interface InstancedMesh2 {
    ...
    /** @internal */ getUniformSchemaResult(
      schema: UniformSchemaShader,
    ): UniformSchemaResult;
    /** @internal */ getUniformOffset(
      size: number,
      tempOffset: number[],
    ): number;
...
InstancedMesh2.prototype.getUniformAt = function (
  id: number,
  name: string,
  target?: UniformValueObj,
): UniformValue {
  if (!this.uniformsTexture) {
    throw new Error(
      'Before get/set uniform, it\'s necessary to use "initUniformsPerInstance".',
    );
  }
  return this.uniformsTexture.getUniformAt(id, name, target);
};

InstancedMesh2.prototype.setUniformAt = function (
  id: number,
  name: string,
  value: UniformValue,
): void {
...
InstancedMesh2.prototype.initUniformsPerInstance = function (
  schema: UniformSchemaShader,
): void {
  if (!this._parentLOD) {
    const { channels, pixelsPerInstance, uniformMap, fetchInFragmentShader } =
      this.getUniformSchemaResult(schema);
...
InstancedMesh2.prototype.getUniformSchemaResult = function (
  schema: UniformSchemaShader,
): UniformSchemaResult {
  let totalSize = 0;
...
InstancedMesh2.prototype.getUniformOffset = function (
  size: number,
  tempOffset: number[],
): number {
  if (size < 4) {
