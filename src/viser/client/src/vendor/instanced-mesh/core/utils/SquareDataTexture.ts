 ...
 */
export function getSquareTextureSize(
  capacity: number,
 ...
 */
export function getSquareTextureInfo(
  ...
  if (channels === 3) {
    console.warn(
      '"channels" cannot be 3. Set to 4. More info: https://github.com/mrdoob/three.js/pull/23228',
    );
    channels = 4;
  }
  ...
  const type: TextureDataType = isFloat
    ? FloatType
    : isUnsignedInt
      ? UnsignedIntType
      : IntType;
...
export class SquareDataTexture extends DataTexture {
  ...
  constructor(
    ...
    const { array, format, size, type } = getSquareTextureInfo(
      arrayType,
      channels,
      pixelsPerInstance,
      capacity,
  ...
  public bindToProgram(
    ...
    programUniforms: unknown,
    materialUniforms: unknown,
    uniformName: string,
  ): void {
    ...
    (renderer.state as any).bindTexture(
      gl.TEXTURE_2D,
  ...
  public update(
    ...
    if (!textureProperties.__webglTexture || sizeChanged) {
      ...
      const slot =
        this.getSlot(materialProperties, uniformName) ??
  ...
  protected updateRows(
    ...
    this._utils ??= new WebGLUtils(
      gl,
      renderer.extensions,
      renderer.capabilities,
    ); // third argument is necessary for older three versions
    ...
    const texturePrimaries =
      this.colorSpace === NoColorSpace
        ? null
        : ColorManagement.getPrimaries(this.colorSpace);
    const unpackConversion =
      this.colorSpace === NoColorSpace || workingPrimaries === texturePrimaries
        ? gl.NONE
   ...
   */
  public getUniformsGLSL(
    textureName: string,
    indexName: string,
    indexType: string,
  ): { vertex: string; fragment: string } {
    const vertex = this.getUniformsVertexGLSL(
      textureName,
      indexName,
      indexType,
    );
    const fragment = this.getUniformsFragmentGLSL(
      textureName,
      indexName,
      indexType,
    );
  ...
  protected getUniformsVertexGLSL(
    textureName: string,
    indexName: string,
    indexType: string,
  ): string {
  ...
  protected getUniformsFragmentGLSL(
    textureName: string,
    indexName: string,
    indexType: string,
  ): string {
