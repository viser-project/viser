  ...
  return async function (moduleArg = {}) {
    ...
    if (ENVIRONMENT_IS_NODE) {
      ...
      if (_scriptName.startsWith("file:")) {
        scriptDirectory =
          nodePath.dirname(require("url").fileURLToPath(_scriptName)) + "/";
      }
      readBinary = (filename) => {
    ...
    function findWasmBinary() {
      if (Module["locateFile"]) {
        return locateFile("Sorter.wasm");
      }
      return new URL("Sorter.wasm", import.meta.url).href;
    }
    ...
    var ___wasm_call_ctors = wasmExports["z"];
    var ___getTypeName = wasmExports["A"];
    var _malloc = wasmExports["C"];
    var _free = wasmExports["D"];
    function run() {
      if (runDependencies > 0) {
