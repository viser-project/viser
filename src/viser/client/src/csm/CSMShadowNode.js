...
class CSMShadowNode extends ShadowBaseNode {
   ...
   */
  _setupFade() {
    const cameraNear = reference("camera.near", "float", this).setGroup(
      renderGroup,
    );
    const cascades = reference("_cascades", "vec2", this)
    ...
    const linearDepth = viewZToOrthographicDepth(
      positionView.z,
      cameraNear,
      shadowFar,
    ).toVar("linearDepth");
    ...
    return Fn((builder) => {
      ...
      for (let i = 0; i < this.cascades; i++) {
        ...
        const closestEdge = linearDepth
          .lessThan(cascadeCenter)
          .select(cascade.x, cascade.y);
        ...
        const inRange = linearDepth
          .greaterThanEqual(csmX)
          .and(linearDepth.lessThanEqual(csmY));

        If(inRange, () => {
          const dist = min(
            linearDepth.sub(csmX),
            csmY.sub(linearDepth),
          ).toVar();
  ...
  _setupStandard() {
    ...
    return Fn((builder) => {
      ...
      for (let i = 0; i < this.cascades; i++) {
        ...
        If(
          linearDepth
            .greaterThanEqual(cascade.x)
            .and(linearDepth.lessThanEqual(cascade.y)),
          () => {
  ...
  updateBefore(/*builder*/) {
    ...
    for (let i = 0; i < frustums.length; i++) {
      ...
      const texelWidth =
        (shadowCam.right - shadowCam.left) / shadow.mapSize.width;
      const texelHeight =
        (shadowCam.top - shadowCam.bottom) / shadow.mapSize.height;
