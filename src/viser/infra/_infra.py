...
class StateSerializer:
    ...
    def serialize(self) -> bytes:
        ...
        for old_index, buf in enumerate(self._binary_buffers):
            digest = hashlib.sha256(buf).digest()
            new_index = index_from_digest.setdefault(digest, len(unique_buffers))
            if new_index == len(unique_buffers):
        ...
        out: list[bytes] = [inner_size.to_bytes(8, "little")]
        for part in parts:
...
def _remap_binary_placeholder_indices(obj: Any, index_remap: dict[int, int]) -> Any:
    """Rewrite the ``__binary_index`` of every binary placeholder (see
    ``Message.as_serializable_dict``) through ``index_remap``. Mutates dicts
    and lists in place; tuples are rebuilt. Returns the resulting object."""
    if isinstance(obj, dict):
        # Match the client's placeholder detection (BinaryMessageDecode.ts):
        # both keys must be present.
        if "__binary_index" in obj and "dtype" in obj:
            obj["__binary_index"] = index_remap[obj["__binary_index"]]
            return obj
        for key, value in obj.items():
            obj[key] = _remap_binary_placeholder_indices(value, index_remap)
        return obj
    if isinstance(obj, list):
        for i, value in enumerate(obj):
            obj[i] = _remap_binary_placeholder_indices(value, index_remap)
        return obj
    if isinstance(obj, tuple):
        return tuple(_remap_binary_placeholder_indices(v, index_remap) for v in obj)
    return obj
