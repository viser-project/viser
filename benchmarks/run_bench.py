...
def print_top(rows: list[dict]) -> None:
    for row in rows:
        print(
...
def run_mode(pw, mode: str, args: argparse.Namespace, out_dir: Path) -> dict:
    ...
    try:
        ...
        info0 = page.evaluate(f"() => {JS_RENDERER_INFO}")
        cdp.send("Profiler.start")
        t0 = time.monotonic()
        page.wait_for_timeout(args.seconds * 1000)
        elapsed = time.monotonic() - t0
        # One round-trip for the in-page counters, then the CDP metrics, and
        # the (slow to serialize) profile last so it doesn't pollute either.
        raf_times, long_tasks, info1 = page.evaluate(
            "() => { window.__bench.stopRaf();"
            f" return [window.__bench.rafTimes, window.__bench.longTasks, {JS_RENDERER_INFO}]; }}"
        )
        ...
        n = len(intervals)
        renders = info1["frame"] - info0["frame"]
        result["steady"] = {
            "elapsed_s": elapsed,
            "raf": {
            ...
            "gl_renders": renders,
            "gl_renders_per_s": renders / elapsed,
            "draw_calls_per_render": info1["calls"],
            "triangles_per_render": info1["triangles"],
            "geometries": info1["geometries"],
            "textures": info1["textures"],
            "programs": info1["programs"],
            "long_tasks": {
...
def main() -> None:
    ...
    with sync_playwright() as pw:
        for mode in modes:
            ...
            load, s = result["load"], result["steady"]
            raf, iv, deltas = s["raf"], s["raf"]["interval_ms"], s["cdp_deltas"]
            print(
                f"  load: {load['nav_to_scene_mounted_s']:.2f}s"
                f" (long tasks: {load['long_tasks_during_load']['total_ms']:.0f}ms,"
                f" script: {load['script_duration_s']:.2f}s)\n"
                f"  fps: {raf['fps']:.1f}"
                f"  interval p50/p95/p99: {iv['p50']:.1f}/{iv['p95']:.1f}/{iv['p99']:.1f} ms\n"
                f"  gl renders/s: {s['gl_renders_per_s']:.1f}"
                f"  draw calls/render: {s['draw_calls_per_render']}"
                f"  programs: {s['programs']}\n"
                f"  main-thread busy: {s['main_thread_busy_pct']:.1f}%"
                f"  script: {deltas['ScriptDuration']:.2f}s"
                f"  layout count: {deltas['LayoutCount']}\n"
            ...
            if args.load_top > 0:
                ...
                print_top(load["cpu_top"][: args.load_top])
            print("  top self-time:")
            print_top(result["cpu_top"][:8])
